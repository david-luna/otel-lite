...
function resolveCredentials() {
  ...
  const rootCertPath = getEnvVar('OTEL_EXPORTER_OTLP_TRACES_CERTIFICATE', 'OTEL_EXPORTER_OTLP_CERTIFICATE');
  ...
  const clientCertPath = getEnvVar('OTEL_EXPORTER_OTLP_TRACES_CLIENT_CERTIFICATE', 'OTEL_EXPORTER_OTLP_CLIENT_CERTIFICATE');
  const clientCert = readFile(clientCertPath);
  const clientKeyPath = getEnvVar('OTEL_EXPORTER_OTLP_TRACES_CLIENT_KEY', 'OTEL_EXPORTER_OTLP_CLIENT_KEY');
  ...
  try {
    ...
    diag.warn('Error creating ssl credentials. Using insecure credentials instead.');
    return grpc.credentials.createInsecure();
  }
}
