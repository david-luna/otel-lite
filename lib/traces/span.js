...
export class Span {
  ...
  constructor(options) {
    ...
    const traceFlags = isParentCtxValid
    ...
    this._span.startTimeUnixNano = timeInputToLong(
  ...
  setAttribute(name, val) {
    ...
    const attribsCount = this._attribsMap.size;
    const attribsLimit = getEnvVar(
      'OTEL_ATTRIBUTE_COUNT_LIMIT',
      'OTEL_SPAN_ATTRIBUTE_COUNT_LIMIT'
    );

    // TODO: log something ???
    if (attribsCount >= attribsLimit) {
      const dropped = this._span.droppedAttributesCount || 0;
      this._span.droppedAttributesCount = dropped + 1;
      return this;
    }

    const lengthLimit = getEnvVar(
      'OTEL_ATTRIBUTE_VALUE_LENGTH_LIMIT',
      'OTEL_SPAN_ATTRIBUTE_VALUE_LENGTH_LIMIT'
    );

    const value = toAnyValue(val, lengthLimit);
  ...
  addEvent(name, attribsOrStart, start) {
    ...
    const eventLimit = getEnvVar('OTEL_SPAN_EVENT_COUNT_LIMIT');
    const eventCount = this._span.events?.length || 0;

    // TODO: log
    if (eventLimit >= eventCount) {
      const dropped = this._span.droppedEventsCount || 0;
      this._span.droppedEventsCount = dropped + 1;
      return this;
    }
    ...
    if (attributes) {
      const attribsLimit = getEnvVar('OTEL_EVENT_ATTRIBUTE_COUNT_LIMIT');
      const lengthLimit = getEnvVar('OTEL_ATTRIBUTE_VALUE_LENGTH_LIMIT'); // is this the right var?
      event.attributes = [];
      for (const [key, val] of Object.entries(attributes)) {
        if (event.attributes.length >= attribsLimit) {
          // TODO: update counter and log
          continue;
        }
        event.attributes.push({key, value: toAnyValue(val, lengthLimit)});
      }
    }
  ...
  addLink(link) {
    ...
    const linksLimit = getEnvVar('OTEL_SPAN_LINK_COUNT_LIMIT');
    const linksCount = this._span.links.length;

    // TODO: log
    if (linksCount >= linksLimit) {
      const dropped = this._span.droppedLinksCount || 0;
      this._span.droppedLinksCount = dropped + 1;
      return this;
    }
    ...
    if (link.attributes) {
      const attribsLimit = getEnvVar('OTEL_LINK_ATTRIBUTE_COUNT_LIMIT');
      linkToAdd.attributes = [];
      for (const [key, val] of Object.entries(link.attributes)) {
        if (linkToAdd.attributes.length >= attribsLimit) {
          // TODO: update counter and log
          continue;
        }
