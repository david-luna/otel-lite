...
async function loadPackage(name) {
  ...
  try {
    ...
    diag.info(`instrumentation "@opentelemetry/instrumentation-${name}" not found`);
  }
...
export async function loadInstrumentations(tracerProvider) {
  ...
  for (const instr of enabled) {
    ...
    if (typeof exports === 'undefined') {
      console.error(`Instrumentation package for ${name} could not be loaded. Check if its installed.`);
      continue;
    }
    ...
    for (const prop of Object.keys(exports)) {
      ...
      // instrumentation
      const instrApi = ['init', 'setMeterProvider', 'setLoggerProvider', 'setTracerProvider'];
